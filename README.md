...
> **通知渠道为广播模型**:配置的所有渠道并发收到每条告警/日报/诊断,失败相互隔离(任一渠道挂只记日志,不影响其余)。投递采用 **send-then-commit**:**只要 ≥1 个渠道投递成功即视为已送达**,据此提交事件状态(去重/冷却);**全部渠道都失败才不提交,留待下一轮重发**——保证瞬时故障不丢事件,也不会因此卡住。单次广播内**不对失败渠道做重试**(下一轮整体重发)。**至少配置一个渠道**(飞书 `FEISHU_VENDOR_WEBHOOK` 或上述任一)即可启动;`FEISHU_VENDOR_WEBHOOK` 不再强制必填,海外自托管可只配 Telegram/ntfy/webhook。
...
WatchMend 内置一个 **localhost-only 的只读 SSR 面板**，把“确定性判定 / 评估≠恢复 / send-then-commit / 只读不执行”四项纪律变成一眼可见的证据：
...
- **安全姿态**：socket 模式与只读标识、env 变量遮蔽计数、启用层、通知渠道；建议命令一律标「永不自动执行」。
