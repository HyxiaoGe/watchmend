...
async def run_docker_scan(
    ...
    *,
    ...
    单容器 inspect_safe() 失败仅记日志并跳过该容器(不入 active_subjects、不中断本轮)。

    emit_oom 在 Prometheus 缺席时为 True:此时 docker 是唯一 OOM 信号源,放行 OOM 规则;
    Prometheus 在场时由 metrics_scan 的 container_restart 覆盖 OOM,这里关闭以免重复出卡。
    """
