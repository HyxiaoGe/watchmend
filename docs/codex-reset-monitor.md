...
元数据），先于公开归档到达的未关联事实也会持久保存，重启后可继续匹配。六小时内存在
多个候选时，仅在最近记录相差不超过 30 分钟、且比其他候选至少近一小时的情况下关联；
否则视为歧义，不自动确认。
...
依赖未变化的试运行修复可使用 `Dockerfile.codex-reset-hotfix`，以已验证运行镜像为基础，
只替换应用源码与安装包目录；无需重新从包仓库下载依赖。默认基础镜像固定为本次修复前的
`watchmend:codex-reset-14a82d7`，可通过 `BASE_IMAGE` 构建参数显式指定兼容的 Python 3.12
基线。常规发版仍使用主 Dockerfile 与锁文件。部署前备份 SQLite、保留停止的旧容器；本次
没有表结构迁移，回滚应用可继续使用现有数据库和发送回执，不必恢复旧数据库导致重复通知。
