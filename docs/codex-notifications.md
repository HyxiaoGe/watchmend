...
15 分钟宽限、取消和最终广播由 WatchMend 后台完成，不阻塞 Codex 代理循环。
...
默认宽限期由 `SENTINEL_CODEX_HOOK_GRACE_SECONDS=900` 控制，长任务阈值由
...
`PostToolUse` 才能观察到操作并取消候选；如果获批工具本身运行超过 15 分钟，仍可能出现一条
...
SENTINEL_CODEX_HOOK_GRACE_SECONDS=900
...
非托管 Hook 在新增或内容变化后不会自动执行。请在 Codex 桌面版的“设置 → 用户配置”中
展开并信任各个 Hook，或在 Codex CLI 中运行 `/hooks` 检查来源和命令后信任当前定义；
信任与 Hook 内容哈希绑定，后续修改需要重新审核。
